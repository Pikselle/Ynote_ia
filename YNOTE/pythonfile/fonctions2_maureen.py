...
def extract_MFCC(y, sr):
    ...
    #print("Taille mfcc_base", mfcc_normalized.shape)
    #print("Taille en byte base", mfcc_normalized.size * mfcc_normalized.itemsize)
    # Réduction de la taille via reshape (exemple)
    #mfcc_resized = moyenne_voisins_groupes(mfcc_normalized,5)

    ### CHANGEMENT LIGNE DESSOUS AJOUT DOWNSAMPLED + PCA
    mfcc_downsampled = mfcc_normalized[:, ::4]
    pca = PCA(n_components=2)
    mfcc_pca= pca.fit_transform(mfcc_downsampled.T)  # Appliquer PCA sur les frames temporelles
    # Retourner la version réduite en taille
    #print("Taille en byte", mfcc_pca.size * mfcc_pca.itemsize)
    #print("Taille S_dB_pca", mfcc_pca.shape)
    return mfcc_pca.tolist()
...
def extract_spectrogram(y, sr):
    ### CHANGEMENT LIGNE DESSOUS HOP_LENGTH
    S = librosa.feature.melspectrogram(y=y, sr=sr, n_mels=8, fmax=500, n_fft=256, hop_length=2048)
    S_dB = librosa.power_to_db(S, ref=np.max)  # Conversion en échelle logarithmique (dB)

    # Réduction de la taille via reshape (exemple)
    #print("S_dB shape:", S_dB.shape)
    # Downsampling temporel pour réduire la taille
    ### CHANGEMENT LIGNE DESSOUS REMPLACEMENT DE RESHAPE PAS DOWNSAMPLED
    S_dB_downsampled = S_dB[:, ::4]
    #print("S_dB_resized shape:", S_dB_downsampled.shape)

    # Appliquer PCA pour réduire la dimensionnalité
    pca = PCA(n_components=2)  # Réduction à 5 composantes principales
    #print("Taille avant PCA:", S_dB_downsampled.T.shape)
    S_dB_pca = pca.fit_transform(S_dB_downsampled.T)  # Appliquer PCA sur les frames temporelles
    #print("Taille après PCA:", S_dB_pca.shape)
...
def analyse_musique():
    ...
    for document in cursor.find({}, {"id": 1, "url": 1}):
        url = document['url']
        id = document['id']
        # Extraire les caractéristiques de chaque musique
